...
function initGame() {
    ...
    restartBtn.classList.remove('hidden'); // 確保開局重置按鈕狀態
...
function endGame(isWin) {
    ...
    if (isWin) {
        ...
        // 🎯 需求 2：踩到地雷失敗時，強制作型絕對隱藏上傳區，只秀出重來按鈕
        gameResultTitle.textContent = "💥 💥 爆炸啦！";
        gameResultText.textContent = "很遺憾，你踩到地雷了。再接再厲！";
        
        uploadScoreZone.classList.add('hidden'); // 徹底修復失敗顯示上傳的 Bug
        restartBtn.classList.remove('hidden');
...
submitScoreBtn.addEventListener('click', () => {
    ...
    database.ref('minesweeper_scores').push({
        ...
        // 💡 需求 3：成績成功上傳後，把原本隱藏的重來按鈕放出來，讓玩家玩下一局
        restartBtn.classList.remove('hidden');
    }).catch((error) => {
